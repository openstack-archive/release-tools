...
if [ "$#" == "0" ] || [ "$1" == "help" ] || [ "$1" == "h" ]
then
    echo "USAGE: ./eol_branch.sh branch eol_tag project [project [project [...]]]"
    echo "Run from workspace containing recent checkouts of project with a remote 'gerrit'"
...
function abandon_reviews {
    ...
    ssh -p 29418 review.openstack.org gerrit query --current-patch-set status:open project:$project branch:$BRANCH |
        grep revision | while read -r line
    do
...
do
    ...
    if ! git rev-parse remotes/$REMOTE/$BRANCH >/dev/null 2>&1
    then
