...
"""Print a list of the repositories with the given tags.
"""
...
def main():
    ...
    for project, info in project_data.items():
        for repo in info['projects']:
            tag_names = set(tag['name'] for tag in repo.get('tags', []))
            if not sought_tags.difference(tag_names):
                print(repo['repo'])
