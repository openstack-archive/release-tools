...
if [ $# -lt 1 ]; then
    echo "Usage: $0 repository [version] SHA"
    echo
    echo "Example: $0 openstack/oslo.rootwrap"
    echo "Example: $0 openstack/oslo.rootwrap 3.0.3"
    exit 2
fi
...
# Clone the repository being released.
#
# FIXME(dhellmann): At some point we'll want to just use the local
# directory, or have an option for that, to make this work in the
# gate.
clone_repo $REPO
REPODIR="$(cd $REPO && pwd)"
cd $REPODIR
